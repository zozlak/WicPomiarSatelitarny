...
#' @details
#' Dokładniej pobierane są wszystkie zdjęcia w rozdzielczości 60 m / piksel.
#' 
...
#' atmosferycznie).
#' @param dlGeogr Długość geograficzna w stopniach. Długość E ma wartości dodatnie, długość W ujemne.
#' @param szGeogr Szerokość geograficzna w stopniach. Szerokość N ma wartości dodatnie, szerokość S ujemne.
#' @param dataOd data początku okresu, dla którego mają być pobrane zdjęcia w formacie RRRR-MM-DD
#' @param dataDo data końca okresu, dla którego mają być pobrane zdjęcia w formacie RRRR-MM-DD
#' @param katalog katalog, do którego zapisane zostaną zdjęcia (domyślnie obecny katalog roboczy)
#' @return ramka danych opisująca pobrane zdjęcia
#' @import dplyr
#' @export
pobierzZdjecia = function(dlGeogr, szGeogr, dataOd, dataDo, progChmur = 50, katalog = '.'){
  req = httr::GET(
  ...
  for (i in seq_along(images$imageId)) {
    ...
    download.file(images$url[i], images$file[i], mode = 'wb', quiet = TRUE)
  }
  cat("\n")
