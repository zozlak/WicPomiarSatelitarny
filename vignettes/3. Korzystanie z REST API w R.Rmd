...
Aby w R wygodnie korzystać z *REST API*, które zwraca dane w formacie JSON, skorzystamy z pakietów:
...
Zainstalujemy je komendą:

```{r, eval=FALSE}
install.packages(c('httr', 'jsonlite', 'dplyr', 'reshape2'))
```

A następnie załadujmy:
...
Wyszukajmy zatem wszystkie paczki danych obejmujących swoim zasięgiem Puławy nie starszych niż 1 lipca 2016 r.
W tym celu skorzystamy *endpoint-u* http://s2.boku.eodc.eu/granule oraz parametrów `utm` (pozwala wskazać element [siatki UTM](https://pl.wikipedia.org/wiki/Uk%C5%82ad_UTM), który mają pokrywać dane) oraz parametru `dateMin` (umożliwiao wskazanie najstarszej dopuszczalnej daty):

```{r}
req = GET(
  'http://s2.boku.eodc.eu/granule', 
  query = list('utm' = '34UEB', 'dateMin' = '2016-07-01'),
  username = 'WIC',
  password = 'pulawy2016'
)
content(req, 'text')
